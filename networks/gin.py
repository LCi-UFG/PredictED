...
class GINMod(nn.Module):
    def __init__(
        ...
        in_dim = combined_dim
        for out_dim in lin_hidden_dims:
    ...
    def forward(
        ...
        Bp = batch_p.max().item() + 1
        pred_vnodes = [emb.expand(Bp, -1) 
        ...
        mol_emb  = (mol_seq  * mol_mask.unsqueeze(-1).float()
            ).sum(dim=1) / mol_mask.sum(dim=1, keepdim=True).clamp_min(1)
        pred_emb = (pred_seq * pred_mask.unsqueeze(-1).float()
            ).sum(dim=1) / pred_mask.sum(dim=1, keepdim=True).clamp_min(1)
